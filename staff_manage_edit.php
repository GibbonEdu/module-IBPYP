...
if (isActionAccessible($guid, $connection2, "/modules/IB PYP/staff_manage_edit.php")==FALSE) {
	...
	if ($ibPYPStaffTeachingID=="") {
		...
		if ($result->rowCount()!=1) {
			...
			$row=$result->fetch() ;
			?>
			<form method="post" action="<? print $_SESSION[$guid]["absoluteURL"] . "/modules/IB PYP/staff_manage_editProcess.php?ibPYPStaffTeachingID=$ibPYPStaffTeachingID" ?>">
					...
					<tr>
						...
						<td class="right">
							<select name="role" id="role" style="width: 302px">
								...
								<option <? if ($row["role"]=="Teacher (Curriculum)") {print "selected ";}?>value="Teacher (Curriculum)">Teacher (Curriculum)</option>
